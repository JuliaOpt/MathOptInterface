...
@testset "Vectorization" begin
    ...
    @testset "operate vcat" begin
        ...
        @testset "Variable with $T" for T in [Int, Float64]
            @test MOI.VectorOfVariables == MOIU.promote_operation(
                vcat,
                T,
                typeof(wf),
                typeof(v),
...
@testset "substitute_variables" begin
    ...
    @test MOIU.eval_variables(vi -> subs_vals[vi], fsa) ==
          MOIU.eval_variables(vi -> vals[vi], subs_sa)
    ...
    @test MOIU.eval_variables(vi -> subs_vals[vi], fva) ==
          MOIU.eval_variables(vi -> vals[vi], subs_va)
    ...
    subs_sq =
        ...
        5.0fy * fz - 2.0
    @test MOIU.eval_variables(vi -> subs_vals[vi], fsq) ==
          MOIU.eval_variables(vi -> vals[vi], subs_sq)
    ...
    subs_vq = MOIU.operate(
        vcat,
        Float64,
        1.0fy - fw - 2.0fy * fw - 3.0,
        2.0fy + 1.0fy * fy - 1.0fw * fy - 1.0fw * fz + 1.0fy * fz - 1.0,
    )
    @test MOIU.eval_variables(vi -> subs_vals[vi], fvq) ==
          MOIU.eval_variables(vi -> vals[vi], subs_vq)
...
@testset "map_indices" begin
    ...
    @test MOIU.constant_vector(gvq) == [-3.0, -2.0]
...
@testset "Scalar" begin
    ...
    @testset "Affine" begin
        ...
        @testset "promote_operation" begin
            @test MOIU.promote_operation(-, Int, MOI.SingleVariable) ==
        ...
        @testset "convert" begin
            ...
            @test MOIU.convert_approx(MOI.SingleVariable, f, tol = 0.5) ==
                  MOI.SingleVariable(x)
        ...
        @testset "operate with Int coefficient type" begin
            ...
            @test f === +f
            @test f ≈
                  MOI.SingleVariable(x) + MOI.ScalarAffineFunction(
    ...
    @testset "Quadratic" begin
        ...
        @testset "operate" begin
            ...
            @test f ≈
                  (fx + 2) * (fx + 1) + (fy + 1) * (2fy + 3fx) + (5 - 3fx - 2fy)
            @test f ≈ begin
...
@testset "Conversion to canonical form" begin
    ...
    @testset "ScalarQuadratic" begin
        ...
        test_canonicalization(1.0x * y + 0.1x * y + 5.0, 1.1x * y + 5.0)
    end
    @testset "VectorQuadratic" begin
        ...
        @test !MOIU.is_canonical(
            MOIU.operate(vcat, Float64, 1.0x * y + 2.0x * x + 2.0),
        )
        @test !MOIU.is_canonical(
            MOIU.operate(vcat, Float64, 1.0x * x + 0.0x * y + 2.0),
        )
...
@testset "Vector operate tests" begin
    ...
    @testset "Vector promote tests" begin
        T = Float64
        for t1 in
            [MOI.VectorAffineFunction{T}, MOI.VectorOfVariables, Vector{T}]
            for t2 in [MOI.VectorAffineFunction{T}, MOI.VectorOfVariables]
                @test MOIU.promote_operation(+, T, t1, t2) ==
    ...
    @testset "Vector + and - Vector" begin
        ...
        @test α + g ≈ g_plus_α
        @test α - v ≈ α_minus_v
        @test MOIU.operate(+, Int, MOIU.operate(-, Int, v, v), v) ≈
              v_minus_v_plus_v
        @test f + α ≈ f_plus_α
        @test f - g ≈ f_minus_g
    ...
    @testset "Vector * and / constant" begin
        v = MOI.VectorOfVariables([y, w, y])
        v2 = MOIU.operate(vcat, Float64, 2.0fy, 2.0fw, 2.0fy)
        f = MOIU.operate(
            vcat,
            Float64,
            2.0fx + 3.0fy + 1.0,
